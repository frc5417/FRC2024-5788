...
public class RedFarSide extends SequentialCommandGroup {
  /** Creates a new TwoNoteCenter. */
  public RedFarSide(TargetStateRun targetStateRun, PhotonSubsystem photon) {
    Pose2d startPose = new Pose2d(15.0, 4.5, Rotation2d.fromDegrees(0));
    Pose2d startPoseCustom = RobotContainer.WPI_to_Custom(startPose);
    Pose2d stupid = RobotContainer.WPI_to_Custom(startPose).times(-1);
    targetStateRun.m_drivebase.resetOdometry(new Pose2d(stupid.getX()*-1, stupid.getY(), stupid.getRotation()));
    

    Pose2d notePose = new Pose2d(8.0, 0.5, new Rotation2d());
    Pose2d notePoseCustom = RobotContainer.WPI_to_Custom(notePose);


    Pose2d[] path_back = A_Star.nodeListToPosesWPI(A_Star.compute(new Node(startPoseCustom.getX(), startPoseCustom.getY(), false), new Node(notePoseCustom.getX(), notePoseCustom.getY(), false)));
    ...
    addCommands(new Fire(1.5).andThen(back));
    /*
    addCommands(
